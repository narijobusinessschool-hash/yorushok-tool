...
export default function DashboardPage() {
  ...
  const [showDiagPopup, setShowDiagPopup] = useState(false);
      ...
      {showDiagPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-2xl">
            <p className="text-lg font-bold text-[#2c2933]">診断して生成精度UPする</p>
            <p className="mt-2 text-sm text-[#6b6380] leading-relaxed">
              あなたの強み・ターゲットを診断すると、<br />AIの文章がより的確になります。
            </p>
            <button
              onClick={() => { setShowDiagPopup(false); router.push("/onboarding"); }}
              className="mt-5 w-full rounded-xl bg-[#e85d8a] py-3 text-sm font-bold text-white transition hover:bg-[#d4507c] active:scale-[0.98]"
            >
              診断する（約3分）
            </button>
            <button
              onClick={() => setShowDiagPopup(false)}
              className="mt-2 w-full py-2 text-sm text-[#9b92a4] transition hover:text-[#2c2933]"
            >
              あとで
            </button>
          </div>
